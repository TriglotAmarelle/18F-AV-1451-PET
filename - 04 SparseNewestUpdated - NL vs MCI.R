...
correlation_subset <- correlation_subset[, -(56)]
...
log.data <- prcomp(scale(check[,10:66]))
...
PTAU <- check$PTAU
ABETA <- check$ABETA
norm <- data.frame(TAU, PTAU, ABETA) #PTAUABETA)
norm <- as.data.frame(lapply(norm[1:3], normalize))
pcafile$TAU <- norm$TAU
pcafile$PTAU <- norm$PTAU
pcafile$ABETA <- norm$ABETA
pcafile <- pcafile[,c(4:15,1:3)]
...
corq1 <- cor(cor_subset[5:63], use="pairwise.complete.obs")
...
p_subset <- check[,-(7:9)]
corq1 <- cor(p_subset[6:63], use="pairwise.complete.obs")
corq1 <- data.frame(corq1)
...
var29 <- which( colnames(check)==besttau[29] )

besttauvect <- c(var29,var20,var21,var22,var23, 
...
df %>%
  ...
  ggplot(aes(x=av1451entorhinal,y=av1451middletemporal,color=prediction)) +
  geom_point() +
...
# So far, with around 63% correct diagnoses for the best regions, and 57% for the worst regions, 
# neither group does a good job
# Let's reduce that to the highest ranked 5 variables for both

var24 <- which( colnames(check)==besttau[29] )
var25 <- which( colnames(check)==besttau[25] )
var26 <- which( colnames(check)==besttau[26] )
var27 <- which( colnames(check)==besttau[27] )
var28 <- which( colnames(check)==besttau[28] )
besttauvect <- c(var29, var25, var26, var27, var28)
clust_df <- check[besttauvect]
clust_df$DX <- check$DX
...
var29 <- which( colnames(check)==bestp[29] )

bestpvect <- c(var19, var20, var21, var22, var23, var24, var25, var26, var27, var28, var29)
...
df %>%
  filter(is.na(DX)) %>%
  mutate(prediction=g_s4@predictions) %>%
  ggplot(aes(x=av1451inferiotemporal,y=av1451totalcort,color=prediction)) +
  geom_point() +
  stat_classifier(color="black", classifiers=list(g_s))
...
# So far, with around 54% correct diagnoses for the best regions, and 55% for the worst regions, 
# neither group does a good job
# Let's reduce that to the highest ranked 5 variables for both

var24 <- which( colnames(check)==bestp[29] )
var25 <- which( colnames(check)==bestp[25] )
var26 <- which( colnames(check)==bestp[26] )
var27 <- which( colnames(check)==bestp[27] )
var28 <- which( colnames(check)==bestp[28] )
bestpvect <- c(var29, var25, var26, var27, var28)
clust_df <- check[bestpvect]
...
df %>%
  filter(is.na(DX)) %>%
  mutate(prediction=g_s4@predictions) %>%
  ggplot(aes(x=av1451entorhinal,y=av1451parahipp,color=prediction)) +
  geom_point() +
  stat_classifier(color="black", classifiers=list(g_s))
...
# CSF predicts just slightly above 54% with SSL

## PCA only

## Let's have a look at PCA variables

pcaregions <- pcafile[-(10:12)]
...
## Okay, let us then only look at the first five variables
...
# PCA elements predict 58% with SSL 
# WINNER

# PCA + CSF
pcacsf <- pcafile[,c(10:12,1:9,13,14,15)]
...
taumaster$ABETA <- norm$ABETA
#taumaster <- taumaster[, -(47)]
